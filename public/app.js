...
async function installVersion(v) {
    ...
    try {
        ...
        document.getElementById('ram-slider').max = m;
        document.getElementById('sys-ram-total').innerText = m.toFixed(1);
    } catch (e) { }
...
async function confirmInstall() {
    ...
    let url = '';
    Toastify({text: 'Obteniendo enlace de descarga...', style:{background:'#3b82f6'}}).showToast();

    try {
        if (v.type === 'vanilla') {
            // Usar resolver del backend para Vanilla
            const res = await api('nebula/resolve-vanilla', { url: v.url });
            if (res && res.url) url = res.url;
            else throw new Error("URL Vanilla no encontrada");
        } 
        else if (v.type === 'paper') {
            const r = await fetch(`https://api.papermc.io/v2/projects/paper/versions/${v.id}`);
            if (!r.ok) throw new Error("Paper API Error");
            ...
            // USAR NUEVO RESOLVER DE FORGE (Backend)
            const res = await api('nebula/resolve-forge', { version: v.id });
            if (res && res.url) url = res.url;
            else throw new Error("Forge no encontrado");
        }
        ...
        if (url) {
            ...
            Toastify({text: 'Descarga iniciada. Revisa la consola.', style:{background:'#10b981'}}).showToast();
        } else {
        ...
        console.error(e);
        Toastify({text: 'Error al obtener versión (Revisa Logs)', style:{background:'#ef4444'}}).showToast();
    }
}
