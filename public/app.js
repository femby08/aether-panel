...
setInterval(() => {
    fetch('/api/stats').then(r => r.json()).then(d => {
        ...
        document.getElementById('cpu-freq').innerText = d.cpu_freq + ' MHz'; // Sin condicional
        ...
        ramChart.data.datasets[0].data.push(parseFloat(ramUsedGB));
        ramChart.update();
        
        document.getElementById('ram-val').innerText = `${ramUsedGB} / ${ramTotalGB} GB`;
        document.getElementById('ram-free').innerText = ramFreeGB + ' GB Libre'; // Sin condicional
...
// --- RESTO DE FUNCIONES (Tienda, Archivos, etc) ---
// (Mismo código de siempre, sin cambios)
...
async function confirmInstall(){ if(!pendingInstall)return; const ram=document.getElementById('ram-slider').value; const v=pendingInstall; closeRamModal(); let url=''; try{ if(v.type==='vanilla'){const r=await api('nebula/resolve-vanilla',{url:v.url});url=r.url} else if(v.type==='paper'){const r=await fetch(`https://api.papermc.io/v2/projects/paper/versions/${v.id}`);const d=await r.json();const b=d.builds[d.builds.length-1];url=`https://api.papermc.io/v2/projects/paper/versions/${v.id}/builds/${b}/downloads/paper-${v.id}-${b}.jar`} else if(v.type==='fabric'){const r=await fetch('https://meta.fabricmc.net/v2/versions/loader');const d=await r.json();url=`https://meta.fabricmc.net/v2/versions/loader/${v.id}/${d[0].version}/1.0.0/server/jar`} else if(v.type==='forge'){url=`https://maven.minecraftforge.net/net/minecraftforge/forge/${v.id}-${v.id}/forge-${v.id}-${v.id}-universal.jar`} if(url){ await api('settings',{ram:ram+'G'}); api('install',{url,filename:'server.jar'}); } }catch(e){Toastify({text:'Error',style:{background:'#ef4444'}}).showToast()} }
