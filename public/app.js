...
// --- 4. MONITOR & CHARTS (GB + Escala Fija) ---
...
// --- RESTO DE LÓGICA (Tienda, Consola, Archivos) ---
...
async function confirmInstall(){ 
    ...
    try{ 
        if(v.type==='vanilla'){
            ...
            if(r && r.url) url=r.url; else throw new Error("URL Vanilla fallida");
        } 
        else if(v.type==='paper'){
            const r=await fetch(`https://api.papermc.io/v2/projects/paper/versions/${v.id}`);
            if(!r.ok) throw new Error("Paper no encontrado");
            ...
            // Forge Installer estandar
            url=`https://maven.minecraftforge.net/net/minecraftforge/forge/${v.id}-${v.id}/forge-${v.id}-${v.id}-installer.jar`;
        } 
        ...
        if(url){ 
            ...
            Toastify({text: 'Descargando...', style:{background:'#10b981'}}).showToast();
        } 
    }catch(e){
        console.error(e);
...
// --- GESTOR DE CONFIGURACIÓN (Con Switches y Alineación) ---
function loadCfg() { 
    api('config').then(d => { 
        ...
        for(const [k, v] of entries) {
            ...
            if(v === 'true' || v === 'false') {
                ...
                const labelText = isChecked ? 'Activado' : 'Desactivado';
                let displayKey = k;
                if(k === 'online-mode') displayKey = 'Modo Premium (Online Mode)';
                
                c.innerHTML += `
                <div class="cfg-item">
                    <label class="cfg-label">${displayKey}</label>
                    <div class="cfg-switch-wrapper">
                        <span style="font-size:0.8rem;color:var(--muted)">${labelText}</span>
                        <label class="switch">
                            <input type="checkbox" class="cfg-bool" data-k="${k}" ${isChecked ? 'checked' : ''} 
