...
// --- MOD INSTALLER API (Añadido V1.3.0) ---
app.post('/api/mods/install', async (req, res) => {
    ...
    // Usamos Wget por seguridad
    exec(`wget -q -O "${target}" "${url}"`, (error) => {
        if (error) {
            io.emit('toast', { type: 'error', msg: 'Error descargando mod' });
...
server.listen(3000, () => console.log('Nebula V1.3.0 running'));const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const path = require('path');
const fs = require('fs');
